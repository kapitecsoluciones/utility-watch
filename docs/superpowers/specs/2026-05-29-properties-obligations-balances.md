...
This mirrors a prior production dashboard (kapilot `utilities/`) whose model is
...
- **C** — one-time import (private, Emmental): `ut_properties` → `properties`;
...
names/balances are Emmental-private (private overlay; never the public repo).
