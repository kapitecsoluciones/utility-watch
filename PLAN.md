...
Utility Watch is built for agents, not only people. The platform's primary interface in v0 is a **Model Context Protocol (MCP) server** so AI agents (Claude Code, ChatGPT, OpenClaw, and others) can discover providers, trigger retrievals, query normalized bills, and pull fresh web data on demand — with full governance and audit.
